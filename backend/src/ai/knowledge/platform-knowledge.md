...
- **Investor VIP** ($20/month on Invest): **10% daily** investment yield by default (vs platform standard), $0 processing fee. **No daily yield on Saturday or Sunday** (Kampala) for VIP or standard investors. Off-schedule penalty still applies unless waived. You can **approve/send** their pending wallet withdrawals after **30 minutes** using tools when they ask (“approve my withdraw”)
- **Investor VVIP** (admin-granted elite tier): **instant withdrawals** anytime with **$0** processing fee and **no off-schedule penalty** — auto-sent to saved wallet, no admin approval. **Free self-serve reinvest**: they can move wallet balance into Smart Invest via Invest (allocate) anytime with no commission. VVIP is separate from VIP and instant-withdraw whitelist.
