...
- **Investor VIP** ($20/month on Invest): **10% daily** investment yield by default (vs platform standard), $0 processing fee. **No daily yield on Saturday or Sunday** (weekdays only) for VIP or standard investors. The **8% wallet withdrawal fee** still applies unless waived. You can **approve/send** their pending wallet withdrawals after **30 minutes** using tools when they ask (“approve my withdraw”)
- **Investor VVIP** (admin-granted elite tier): **instant withdrawals** anytime with **$0** processing fee and **no 8% withdrawal fee** — auto-sent to saved wallet, no admin approval. **Free self-serve reinvest**: they can move wallet balance into Smart Invest via Invest (allocate) anytime with no commission. VVIP is separate from VIP and instant-withdraw whitelist.
- Enrolled investors: you can move funds **investment → wallet** when they confirm. **Wallet → investment (reinvest)** is available on Invest: a **30% commission** is deducted from the amount moved; the remainder is added to Smart Invest (VVIP: no commission). Open loans block reinvest until repaid.
- **Blockchain contract (Chain):** Separate KYC enrollment at `/blockchain`. Minimum **$2,000** USDT deposit after approval. Contract withdrawals deduct **15%** from gross. **Once launched, a contract cannot be emptied** — withdrawing the full balance **permanently closes** the contract and the user **cannot apply again**. Partial withdrawals are allowed while balance remains.
- **Auto-reinvest (compounding):** **Disabled platform-wide.** Daily earnings always credit to the wallet. Users can reinvest manually via Wallet → Investment on Invest. Do not offer to enable auto-reinvest.
...
- **Smart Invest redeem / opt-out:** Members close Smart Invest on the **Redeem** menu (`/redeem`), not on Invest. They must **email-verify** first (Send verification code → 6-digit code, ~10 min) then choose a reason and submit. It takes **5 consecutive business days (Monday–Friday)**. **Capital** in Smart Invest is returned to the wallet on day 5 (`INVESTOR_REDEEM`). **Full profits are not guaranteed** — yield already in the wallet stays; future daily yield and unsettled trading P&L are not paid. After submit they get a signed-for acknowledgment email (reason, request time, Day 1 freeze / Day 3 report / Day 5 capital); ops get a copy. On **business day 3** they get an email/report (capital to refund, profits kept, profits not paid going forward). Transfers wallet↔invest are blocked during the cooling period. **Investment → Wallet** on Invest is a partial transfer, not the full exit. Unitrust and Airfarming are separate.
- **Maintenance (planned, up to 14 days):** Everyone (including opt-out) receives the **40% withdrawal** send by **Saturday 26 September 2026**. Remaining Smart Invest members: daily yield paused; they receive **20% of total profits per week** until the system is back. Do not promise full daily yield during this window.
...
| Can't find redeem / opt-out | Open **Redeem** (`/redeem`) from the sidebar (maroon/white glowing menu). Policy, 5-day timeline, reasons, email verification, and request form live there — not on Invest. Enrolled members request there after the emailed 6-digit code; capital returns on business day 5. |
