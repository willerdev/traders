/**
 * One-off: move full wallet balance → Smart Invest for moisentak@gmail.com (admin transfer, no fee).
 * Usage: cd backend && npx tsx scripts/transfer-moisentak-wallet-to-invest.ts
 */
...
const USER_ID = 'cmrwlprcu09mlqa01wq1xapev';
const ADMIN_ID = 'cmqmtehqi0000wfaxxntkiua9';
const EMAIL = 'moisentak@gmail.com';
...
async function main() {
  ...
  console.log(
    JSON.stringify(
      {
        email: user.email,
        method: 'admin transfer (InvestorService.transferInvestment with adminId)',
        amountMoved: rounded,
        feePercent,
        feeAmount,
        netInvested,
